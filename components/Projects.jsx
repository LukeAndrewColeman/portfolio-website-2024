  ...
  return (
    <section id="projects" className="py-20 scroll-mt-20">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <h2 className="text-4xl font-bold text-center mb-16 text-luke-light-blue tracking-tight">Personal Projects</h2>
        <div class="flex gap-6 flex-wrap justify-center">
          {projects.map((project, index) => (
              <div className="bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl group flex flex-col max-w-[400px]">
                <div className="relative h-48">
                  <Image src={project.image} alt={project.title} layout="fill" objectFit="cover" />
                  <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <Link href={project.link} target="_blank" rel="noopener noreferrer">
                      <button className="py-2 px-4 bg-luke-green hover:bg-luke-hover transition-colors duration-300 text-white rounded-full">View Project</button>
                    </Link>
                  </div>
                </div>
                <div className="p-6 flex flex-col flex-grow overflow-auto">
                  <h3 className="text-2xl font-bold mb-3 text-luke-green">{project.title}</h3>
                  <p className="text-luke-dark-blue mb-4">{project.description}</p>
                  <div className="flex flex-wrap gap-2 mt-auto pt-4">
                    {project.technologies.map((tech, i) => (
                      <span key={i} className="bg-luke-light-blue bg-opacity-10 text-luke-light-blue px-3 py-1 rounded-full text-xs font-medium">
                        {tech}
                      </span>
                    ))}
                  </div>
